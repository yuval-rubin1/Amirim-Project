...
for i in range(1, num_iterations):
    ...
    # rR[:, i] = rR[:, i-1] + dt*(1 / tau_r)*(-rR[:, i-1] + (SR[:, i-1]-SL[:, i-1]+ outside_input)*xi + r0)
    # rL[:, i] = rL[:, i-1] + dt*(1 / tau_r)*(-rL[:, i-1] - (SR[:, i-1]-SL[:, i-1] + outside_input)*xi + r0)
    ...
    # add_to_spike_cnt_R = np.random.rand(len(xi)) < (rR[:, i] * dt * redIO*100)
    # add_to_spike_cnt_L = np.random.rand(len(xi)) < (rL[:, i] * dt * redIO*100)
    ...
    # SR[:, i] = SR[:, i-1] + dt*(1 / tau_s)*(-SR[:, i-1] + np.dot(rR_activated, eta))
    # SL[:, i] = SL[:, i-1] + dt*(1 / tau_s)*(-SL[:, i-1] + np.dot(rL_activated, eta))
